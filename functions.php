<?php 
    ...
    if (isset($_POST['comment_posted'])) {
        ...
        if ($result) {
            $comments = "<div class='comment clearfix'>
                        ...
                        <form action='post_details.php' class='reply_form clearfix' id='comment_reply_form_" . $inserted_comment['id'] . "' data-id='" . $inserted_comment['id'] . "'>
                            <textarea class='form-control' name='reply_text' id='reply_text' cols='3040' rows='2'></textarea>
    ...
    if (isset($_POST['replies'])) {
        ...
        if ($result) {
            $replies = "<div class='comment reply clearfix'>
                        <img src='profile.png' alt='' class='profile_pic'>
                        <div class='comment-details'>
                            <span class='comment-name'>" . getUsernameById($inserted_replies['user_id']) . "</span>
                            <span class='comment-date'>" . date('F j, Y ', strtotime($inserted_replies['created_at'])) . "</span>
                            <p>" . $inserted_replies['body'] . "</p>
                            <a class='reply-btn' href='post_details.php'>reply</a>
                        </div>
                    </div>";
            echo $reply;
            exit();
